...
with
    ...
    member this.Length = FingerTree.length this
    ...
    override this.Equals (other: obj): bool =
        ...
        | :? FingerTree<'t> as other ->
            let rec compare a b =
                match FingerTree.viewV a with
                | ValueNone ->
                    match FingerTree.viewV b with
                    | ValueNone -> true
                    | ValueSome _ -> false
...
module FingerTree =
    ...
    module private Internals =
        ...
        let inline foldInternal ([<InlineIfLambda>] folder: 'state -> 't -> 'state) (state: 'state) (tree: FingerTree<'t>) : 'state =
            match viewV tree with
            | ValueNone -> state
            | ValueSome (value, rest) ->
                foldInternal folder (folder state value) rest
        let inline foldBackInternal ([<InlineIfLambda>] folder) (tree: 't FingerTree) (state: 'state) =
            match viewRevV tree with
            | ValueNone -> state
            | ValueSome (value, rest) ->
                foldBackInternal folder rest (folder value state)
        
        let inline reduceInternal ([<InlineIfLambda>] folder: 't -> 't -> 't) (tree: FingerTree<'t>) =
            match viewV tree with
            | ValueNone -> invalidArgCollectionWasEmpty()
            | ValueSome (value, rest) ->
                foldInternal folder value rest
        let inline reduceBackInternal ([<InlineIfLambda>] folder: 't -> 't -> 't) (tree: FingerTree<'t>) =
            match viewRevV tree with
            | ValueNone -> invalidArgCollectionWasEmpty()
            | ValueSome (value, rest) ->
                foldBackInternal folder rest value
        
        let rightInternal tree =
            match tree with
            | Blank -> None
            | Single x -> Some x
        ...
        let inline mapFoldInternal ([<InlineIfLambda>] mapping: 'state -> 't -> 'u * 'state) (state: 'state) (tree: FingerTree<'t>) =
            foldInternal (fun (result, state) item ->
                let item, newState = mapping state item
                (insertRight item result, newState)
            ) (Blank, state) tree
        let inline mapFoldBackInternal ([<InlineIfLambda>] mapping) (tree: 't FingerTree) (state: 'state) =
            foldBackInternal (fun item (result, state) ->
                let item, newState = mapping state item
                (insertLeft item result, newState)
            ) tree (Blank, state)
            
        let toArrayInternal tree =
            [|
    ...
    let chunkBySize chunkSize source =
        let result, index =
            source
            |> foldInternal (fun (result, index) item ->
                match rightInternal result with
                | Some (group: 't[]) ->
        ...
        match popRightInternal result with
        | Some group, rest ->
            if index < group.Length then
    ...
    let collect mapping source =
        source
        |> foldInternal (fun result item -> append result (mapping item)) empty
    ...
    let distinct tree =
        tree
        |> foldInternal (fun (result, seen) item ->
    ...
    /// <param name="tree">The input tree.</param>
    let inline fold ([<InlineIfLambda>] folder: 'state -> 't -> 'state) (state: 'state) (tree: FingerTree<'t>) : 'state = foldInternal folder state tree
    ...
    /// <param name="state">The initial state.</param>
    /// <param name="tree">The input tree.</param>
    let inline foldBack (folder: 't -> 'state -> 'state) (tree: FingerTree<'t>) (state: 'state) =
        foldBackInternal folder tree state
    ...
    let indexed tree =
        tree
        |> mapFoldInternal (fun index item -> (index, item), (index + 1)) 0
    ...
    /// <param name="tree">The input tree.</param>
    let rec inline iter ([<InlineIfLambda>] action: 't -> unit) (tree: FingerTree<'t>) =
        match viewV tree with
        | ValueNone -> ()
        | ValueSome (value, rest) ->
            action value
    ...
    let mapFold (mapping: 'state -> 't -> 'u * 'state) (state: 'state) (tree: FingerTree<'t>) = mapFoldInternal mapping state tree
    ...
    /// <param name="tree">The input tree.</param>
    /// <returns>A pairing of the transformed elements and the final state.</returns>
    let mapFoldBack (mapping: 'state -> 't -> 'u * 'state) (tree: FingerTree<'t>) (state: 'state) = mapFoldBackInternal mapping tree state
    ...
    let mapi mapping tree =
        ...
        tree
        |> mapFoldInternal (fun index item ->
    ...
    let max tree = reduceInternal Operators.max tree
    ...
    let maxBy projection tree = reduceInternal (fun a b -> Operators.max (projection a) (projection b)) tree
    ...
    let min tree = reduceInternal Operators.min tree
    ...
    let minBy projection tree = reduceInternal (fun a b -> Operators.min (projection a) (projection b)) tree
    ...
    let partition (condition: 't -> bool) (tree: FingerTree<'t>) =
        let rec partitionInternal condition evTrue evFalse tree =
            match viewV tree with
            | ValueNone -> evTrue, evFalse
            | ValueSome (item, rest) ->
                if condition item then
                    partitionInternal condition (insertRight item evTrue) evFalse rest
                else
                    partitionInternal condition evTrue (insertRight item evFalse) rest
        
        partitionInternal condition Blank Blank tree
    ...
    /// <param name="tree">The input tree.</param>
    /// <returns>The final state.</returns>
    let reduce reduction tree = reduceInternal reduction tree
    ...
    /// <param name="tree">The input tree.</param>
    /// <returns>The final state.</returns>
    let reduceBack reduction tree = reduceBackInternal reduction tree
    ...
    let inline sum (tree: 't FingerTree) : 't when 't: (static member (+): 't * 't -> 't) and 't: (static member Zero: 't) =
        tree
        |> foldInternal (fun total item -> total + item) 't.Zero
    ...
    let inline sumBy (projection: 't -> 'u) (tree: 't FingerTree) : 'u when 'u: (static member (+): 'u * 'u -> 'u) and 'u: (static member Zero: 'u) =
        tree
        |> foldInternal (fun total item -> total + (projection item)) 'u.Zero
    ...
    let toArray tree = toArrayInternal tree
    ...
    let truncate count tree =
        if count = 0 then tree
        else
            match viewV tree with
            | ValueNone -> empty
            | ValueSome (_, rest) ->
                truncate (count - 1) rest
    ...
    /// <summary>
    /// Returns the index of the last item for which the predicate returns true or <c>ValueNone</c>. 
    /// </summary>
    /// <param name="predicate">The predicate to evaluate each item with.</param>
    /// <param name="tree">The input tree.</param>
    /// <returns>The index of the last item for which the predicate returns true or <c>ValueNone</c>.</returns>
    let tryFindIndexBackV predicate tree =
        tree
        |> toArray
        |> Array.tryFindIndexBackV predicate
    ...
    /// </returns>
    let zip tree1 tree2 =
        let rec inline go current tree1 tree2 =
            match viewV tree1, viewV tree2 with
            | ValueNone, ValueNone -> current
            | ValueNone, _ | _, ValueNone ->
                invalidArgNotOfEqualLength "tree"
    ...
    let zip3 tree1 tree2 tree3 =
        let rec inline go current tree1 tree2 tree3 =
            match viewV tree1, viewV tree2, viewV tree3 with
            | ValueNone, ValueNone, ValueNone -> current
            | ValueNone, _, _ | _, ValueNone, _ | _, _, ValueNone ->
                invalidArgNotOfEqualLength "tree"
    ...
    let removeAt index source =
        source
        |> filteri (fun itemIndex _ -> itemIndex <> index)
    ...
    let removeManyAt index count source =
        source
        |> filteri (fun itemIndex _ -> itemIndex < index || itemIndex >= index + count)
    ...
    let insertAt index value tree =
        tree
        |> foldInternal (fun (itemIndex, result) existing ->
    ...
    /// <exception cref="System.ArgumentException">Thrown when count is less than 0.</exception>
    let randomChoicesWith random count source =
        ensureNonNegative (nameof count) count
        let items = toArrayInternal source
    ...
    /// <param name="source">The input tree.</param>
    /// <returns>A tree of randomly selected elements from the input tree.</returns>
    let randomSampleWith random count source =
        let items = toArrayInternal source
    ...
    let filteri predicate tree =
        tree
        |> foldInternal (fun (index, result) item ->
    ...
    let right (tree: FingerTree<'t>) : 't option = rightInternal tree
    /// <summary>
    /// Attempts to return the rightmost item of the tree.
    /// </summary>
    /// <param name="tree">The input tree.</param>
    /// <returns>The rightmost item or <c>ValueNone</c> if the tree is empty.</returns>
    let rightV tree = rightInternalV tree
    ...
    let popRight (tree: FingerTree<'t>) = popRightInternal tree
    /// <summary>
    /// Returns the rightmost leaf of the tree and a new tree that is missing that leaf.
    /// </summary>
    /// <param name="tree">The input tree.</param>
    /// <returns>The rightmost leaf of the tree and a new tree that is missing that leaf.</returns>
    let popRightV (tree: FingerTree<'t>) = popRightInternalV tree
